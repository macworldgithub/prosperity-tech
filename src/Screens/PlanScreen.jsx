...
export default function PlansScreen() {
  ...
  const handlePaymentProcessed = async (result) => {
    ...
    if (result?.success && selectedPlan) {
      try {
        // Reverted to original endpoint and added body if needed; adjust based on API
        // Assuming the original was intended to subscribe/activate the plan for the customer
        // Added customerNo to the body if the API expects it
        const patchBody = {
          customerNo: customerNo,
          // Add other required fields if known
        };
        const response = await axios.patch(
          `${API_BASE_URL}api/v1/plans/${selectedPlan.planNo}`,
          patchBody,
        ...
        if (updateData) {
          setCurrentPlan({
            planNo: updateData.planNo || selectedPlan.planNo,
            planName: updateData.planName || selectedPlan.planName,
          });
          Alert.alert(
            "Success",
            `Plan updated successfully! New Plan: ${selectedPlan.planName} (No: ${selectedPlan.planNo})`
          );
          fetchPlans(); // Refresh
        }
      } catch (err) {
        console.error("Error updating plan:", err.response?.data || err);
        Alert.alert(
          "Error",
          "Payment successful but plan update failed. Please contact support."
        );
