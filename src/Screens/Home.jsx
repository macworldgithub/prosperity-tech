...
import { SafeAreaView } from "react-native-safe-area-context";
import tw from "tailwind-react-native-classnames";
import { ArrowRight, FileText, MapPin, Wifi } from "lucide-react-native";
import { useNavigation } from "@react-navigation/native";
import Icon from "react-native-vector-icons/Feather";
import { theme } from "../utils/theme";
import AsyncStorage from "@react-native-async-storage/async-storage";
...
export default function Home() {
  ...
  const fetchMobileBalance = async (custNo) => {
    try {
      ...
      const token = await AsyncStorage.getItem("access_token");
      const response = await axios.get(
        `${API_BASE_URL}api/v1/customers/${custNo}/balance/mobile?lineSeqNo=1`,
        {
          headers: {
            accept: "*/*",
      ...
      setMobileBalance(response.data.data || null);
    } catch (err) {
  ...
  // Accurate Data Usage from Mobile Balance
  const getPlanData = () => {
    ...
    for (let item of mobileBalance.queryItems) {
      if (item.unitCode === "Data" && item.accountDesc?.includes("Plan")) {
        ...
        if (match) {
          ...
          const percentageUsed = Math.min(100, (usedGB / totalGB) * 100);
          return {
            totalGB,
  ...
  const totalRemainingDataGB = mobileBalance
    ? (
        mobileBalance.queryItems
  ...
  const handleChangePin = async () => {
    if (!oldPin || !newPin || newPin.length < 4) {
      Alert.alert("Error", "Please enter valid old and new PIN (min 4 digits)");
      return;
    }
    try {
  ...
  const handleCloseAccount = async () => {
    Alert.alert("Close Account", "This action is permanent. Are you sure?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Close Account",
        style: "destructive",
        onPress: async () => {
          ...
          try {
            ...
            Alert.alert("Success", "Account closure request sent.");
            await AsyncStorage.multiRemove([
              "access_token",
              "userData",
              "lastEmail",
      ...
      >
          ...
          <View style={tw`flex-row justify-between`}>
            <View>
              ...
              <Text
                style={tw`text-green-600 font-medium border border-green-600 bg-green-50 rounded-full px-3 py-1 -ml-2`}
              >
                {user?.category_status_customer || "Active"}
              </Text>
            </View>
            <View style={tw`items-end`}>
              <Text style={tw`text-gray-500`}>Service Address</Text>
              <Text style={tw`text-right font-medium text-black w-36`}>
                {user?.serviceAddress || "N/A"}
              </Text>
            </View>
          </View>
        </View>

        {/* Data Usage - Real Data */}
        <View
          style={tw`bg-white mx-2 p-4 rounded-xl border border-gray-200 mb-4`}
        >
          <Text style={tw`font-semibold mb-2`}>Data Usage</Text>
            ...
            <>
              ...
              <View style={tw`w-full bg-gray-200 h-2 rounded-full mt-2`}>
                <View
                  style={[
                    tw`h-2 rounded-full`,
                    { backgroundColor: theme.colors.secondary, width: "0%" },
                  ]}
                />
              </View>
            </>
        ...
        >
          ...
          {balanceLoading ? (
            ...
            <Text style={tw`text-gray-500`}>No balance data</Text>
          )}

          <TouchableOpacity
            onPress={() => navigation.navigate("ChatAI")}
            style={[
              tw`py-3 rounded-xl mt-4 mb-6`,
              { backgroundColor: theme.colors.primary },
            ]}
          >
            <Text style={tw`text-center text-white font-semibold`}>
          ...
          <TouchableOpacity
            ...
            style={tw`bg-white p-4 rounded-xl border ${
              closingAccount ? "border-gray-300" : "border-red-200"
            } flex-row justify-between items-center mb-3`}
            onPress={handleCloseAccount}
          >
            <View style={tw`flex-row items-center`}>
              <Icon name="user-x" size={22} color="red" style={tw`mr-3`} />
              <View>
                <Text style={tw`text-black font-medium`}>Close My Account</Text>
                <Text style={tw`text-gray-500 text-xs mt-1`}>
